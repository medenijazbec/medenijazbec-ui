...
import styles from "./VGRegister.module.css";
...
export default function VGRegister() {
  ...
  const canSubmit = Boolean(email.trim()) && pw.length >= 6 && pw === pw2 && !busy;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setMsg("");
    try {
      const res = await http.post<AuthResponse>("/api/auth/register", { email, password: pw });
      ...
      setMsg(typeof apiErr === "string" ? apiErr : "Registration failed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className={styles.page}>
      <form className={styles.card} onSubmit={onSubmit} aria-label="Register">
                <div
...
/>
        ...
        <p className={styles.sub}>
          Hidden portal • <span className={styles.hint}>/register</span>
        </p>
        ...
        <div className={styles.links}>
          ...
          <Link to="/vs" className={styles.link}>Secret login</Link>
        </div>
